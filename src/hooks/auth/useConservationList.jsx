...
const useConservationList = () => {
    const getKey = (pageIndex, previousPageData) => {
        ...
        if (previousPageData && previousPageData.length > 0) {
            ...
            const previousTweets = previousPageData[0].result.receivers; // Lấy tweets từ trang trước đó
        ...
        return `/api/message/receivers?page=${pageIndex + 1}&limit=${PAGE_SIZE}`;
    };
    ...
    const receivers = data ? data.flatMap((page) => page.result?.receivers || []) : [];
    const sortedReceivers = receivers.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
...
export default useConservationList;
